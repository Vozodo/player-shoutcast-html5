...
$statusUrl = filter_input(INPUT_GET, 'statusurl');
		...
		if (!empty($data)) {
			...
			if (isset($ice_stats["icestats"]["source"])) {
				...
				if (isset($ice_stats_source["bitrate"])) {
					...
					if (isset($ice_stats_source["title"]) || isset($ice_stats_source["yp_currently_playing"])) {

						if (isset($ice_stats_source["yp_currently_playing"])) {
						...
						if (
							isset($currently_playing[0]) && isset($currently_playing[1]) &&
